...
# Condense everything to 3y:


amk2<-list()
daml2<-list()
autopolity2<-matrix(0,nrow = 224,ncol=22)
GDPimp2<-matrix(0,nrow = 224,ncol=22)
military_ex2<-matrix(0,nrow = 224,ncol=22)
nEX2<-matrix(0,nrow = 257,ncol=22)
up<-1
for (t in seq(2,67,by=3)){
  amk2[[up]]<-amk[[t]]+amk[[t+1]]+amk[[t+2]]
  daml2[[up]]<-daml[[t]]*daml[[t+1]]*daml[[t+2]]
  autopolity2[,up]<-autopolity[,t]/3+autopolity[,t+1]/3+autopolity[,t+2]/3
  GDPimp2[,up]<-GDPimp[,t]/3+GDPimp[,t+1]/3+GDPimp[,t+2]/3
  military_ex2[,up]<-military_ex[,t]/3+military_ex[,t+1]/3+military_ex[,t+2]/3
  nEX2[,up]<-nEX[,t]*nEX[,t+1]*nEX[,t+2]
...
daml<-daml2
rm(daml2)
autopolity<-autopolity2
rm(autopolity2)
GDPimp<-GDPimp2
rm(GDPimp2)
military_ex<-military_ex2
rm(military_ex2)
nEX<-nEX2
rm(nEX2)
...
save.image("Datasets/Data_ready_for_regression_3ywindow.RData")
