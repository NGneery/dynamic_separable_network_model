...
# Condense everything to 2y:


amk2<-list()
daml2<-list()
autopolity2<-matrix(0,nrow = 224,ncol=33)
GDPimp2<-matrix(0,nrow = 224,ncol=33)
military_ex2<-matrix(0,nrow = 224,ncol=33)
nEX2<-matrix(0,nrow = 257,ncol=33)
up<-1
for (t in seq(2,67,by=2)){
  amk2[[up]]<-amk[[t]]+amk[[t+1]]
  daml2[[up]]<-daml[[t]]*daml[[t+1]]
  autopolity2[,up]<-0.5*autopolity[,t]+0.5*autopolity[,t+1]
  GDPimp2[,up]<-0.5*GDPimp[,t]+0.5*GDPimp[,t+1]
  military_ex2[,up]<-0.5*military_ex[,t]+0.5*military_ex[,t+1]
  nEX2[,up]<-nEX[,t]*nEX[,t+1]
...
save.image("Datasets/Data_ready_for_regression_2ywindow.RData")
